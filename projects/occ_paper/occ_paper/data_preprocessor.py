...
class SccDataPreprocessor(Det3DDataPreprocessor):
    ...
    def simple_process(self, data: dict, training: bool = False) -> dict:
        ...
        if "range_img" in data["inputs"]:
            batch_pad_shape_range = self._get_pad_shape_range(data)
        ...
        if "range_imgs" in inputs:
            imgs = inputs["range_imgs"]
            ...
            if data_samples is not None:
                ...
                # is then used for the transformer_head.
                batch_input_shape = tuple(imgs[0].size()[-2:])
                for data_sample, pad_shape in zip(data_samples, batch_pad_shape_range):
                    data_sample.set_metainfo({"batch_input_shape_range": batch_input_shape, "pad_shape_range": pad_shape})
        ...
        if self._enable_normalize:
            if self.mean.shape[0] == 3:
                assert _batch_img.dim() == 3 and _batch_img.shape[0] == 3, (
                    "If the mean has 3 values, the input tensor " "should in shape of (3, H, W), but got the " f"tensor with shape {_batch_img.shape}"
                )
        ...
        if "img" in data["inputs"]:
            ...
            if is_seq_of(_batch_imgs, torch.Tensor):
                ...
                if img_dim == 3:
                    ...
                    batch_imgs = multiview_img_stack_batch(batch_imgs, self.pad_size_divisor, self.pad_value)
            ...
            # Process data with `default_collate`.
                ...
                raise TypeError(
                    "Output of `cast_data` should be a list of dict " "or a tuple with inputs and data_samples, but got " f"{type(data)}: {data}"
                )

            data["inputs"]["imgs"] = batch_imgs

        if "range_img" in data["inputs"]:
            _batch_imgs = data["inputs"]["range_img"]
            # Process data with `pseudo_collate`.
            if is_seq_of(_batch_imgs, torch.Tensor):
                batch_imgs = []
                img_dim = _batch_imgs[0].dim()
            ...
            data["inputs"]["range_imgs"] = batch_imgs
    ...
    def _get_pad_shape_range(self, data: dict) -> List[Tuple[int, int]]:
        """Get the pad_shape of each image based on data and
        pad_size_divisor."""
        # rewrite `_get_pad_shape` for obtaining image inputs.
        _batch_inputs = data["inputs"]["range_img"]
        # Process data with `pseudo_collate`.
        if is_seq_of(_batch_inputs, torch.Tensor):
            batch_pad_shape = []
            for ori_input in _batch_inputs:
