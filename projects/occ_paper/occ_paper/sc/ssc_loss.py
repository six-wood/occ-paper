...
def precision_loss(pred, ssc_target, ignore_index=255):
    # Get softmax probabilities
    pred = F.softmax(pred, dim=1)
...
def CE_ssc_loss(pred, target, class_weights, ignore_index=255):
    criterion = nn.CrossEntropyLoss(weight=class_weights, ignore_index=ignore_index, reduction="none")
    loss = criterion(pred, target.long())
    loss_valid = loss[target != ignore_index]
    loss_valid_mean = torch.mean(loss_valid)
    return loss_valid_mean


def BCE_ssc_loss(pred, target, class_weights, alpha, ignore_index=255):
